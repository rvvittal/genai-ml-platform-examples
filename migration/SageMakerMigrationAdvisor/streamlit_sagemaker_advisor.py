...
class SageMakerAdvisorApp:
    ...
    def write_to_file(self, interaction: Dict[str, Any]):
        ...
        output_file = "streamlit_agent_interactions.txt"
        separator = "=" * 80
    ...
    def handle_architecture_input(self):
        ...
        if has_diagram == "Yes":
            ...
            if uploaded_file is not None:
                ...
                st.image(image, caption="Uploaded Architecture Diagram", width="column")
    ...
    def handle_diagram_step(self):
                ...
                if st.button("🎨 Generate Architecture Diagram", help="Generate visual architecture diagrams"):
                    try:
                        ...
                        with domain_name_tools:
                            ...
                            self.save_interaction('Diagram Agent', diagram_input, str(response), 'diagram')
                            st.session_state.workflow_state['completed_steps'].append('diagram')
                            st.session_state.workflow_state['current_step'] = 'tco'
                            
                            progress.empty()
                            st.success("✅ Diagram generated! Check the generated-diagrams folder.")
                            st.rerun()
        ...
        if diagram_response:
            ...
            if os.path.exists('generated-diagrams'):
                diagram_files = [f for f in os.listdir('generated-diagrams') if f.endswith(('.png', '.jpg', '.jpeg'))]
                if diagram_files:
                    st.markdown("**Generated Diagrams:**")
                    for diagram_file in diagram_files:
                        try:
                            img_path = os.path.join('generated-diagrams', diagram_file)
                            st.image(img_path, caption=diagram_file, width="column")
                        except Exception as e:
                            st.warning(f"Could not display {diagram_file}: {e}")
